...
本検証では、FSR上に構築した Windows Server 2019 を実行環境とし、そこから  
...
・使用ツール: Terraform (IIJ P2 Gen.2 フレキシブルサーバリソース API)

 ### 構成ファイル
・"auth.tf": APIトークンを使用したプロバイダー（接続先）の設定  
・"create.tf": vAppおよび仮想マシン（Windows 2022）の定義
...
5. 認証のための設定ファイルと（auth.tf）インフラ環境構築のための設定ファイル（create.tf）を作成
